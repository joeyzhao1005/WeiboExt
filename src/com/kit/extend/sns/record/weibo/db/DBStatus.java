...
public class DBStatus extends DB {
    ...
    public Cursor selectOrderBy(String where, String column, String order) {
        // SQLiteDatabase mDb =mDbhis.getReadableDatabase();
     ...
     * @return
     */
    public Cursor selectOrderBy(String where, int page, int count, String column, String order) {
        // SQLiteDatabase mDb =mDbhis.getReadableDatabase();
        //System.out.println(where);

        Cursor cursor = mDb.query(TABLE_NAME, null, where, null, null, null,
                column + " " + order, (page - 1) * count + "," + count);
    ...
    public StatusWapper selectByIdReturnStatus(long id) {
        ...
        String userId = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_USERID));
        ...
        String createdAt = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_CREATEDAT));
        String mid = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_MID));
        String idstr = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_IDSTR));
        String text = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_TEXT));
        String favorited = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_FAVORITED));
        String truncated = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_TRUNCATED));
        String inReplyToStatusId = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_INREPLYTOSTATUSID));
        String inReplyToUserId = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_INREPLYTOUSERID));
        String inReplyToScreenName = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_INREPLYTOSCREENNAME));
        String thumbnailPic = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_THUMBNAILPIC));
        String bmiddlePic = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_BMIDDLEPIC));
        String originalPic = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_ORIGINALPIC));
        long retweetedStatusId = cursor.getLong(cursor
                .getColumnIndexOrThrow(FIELD_RETWEETEDSTATUSID));
        String geo = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_GEO));
        String latitude = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_LATITUDE));
        String longitude = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_LONGITUDE));
        int repostsCount = cursor.getInt(cursor
                .getColumnIndexOrThrow(FIELD_REPOSTSCOUNT));
        int commentsCount = cursor.getInt(cursor
                .getColumnIndexOrThrow(FIELD_COMMENTSCOUNT));
        String annotations = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_ANNOTATIONS));
        int mlevel = cursor.getInt(cursor
                .getColumnIndexOrThrow(FIELD_MLEVEL));

        String sourceUrl = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_ANNOTATIONS));
        String sourceRelationShip = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_SOURCERELATIONSHIP));
        String sourceName = cursor.getString(cursor
                .getColumnIndexOrThrow(FIELD_SOURCENAME));
        ...
        int visibleType = cursor.getInt(cursor
                .getColumnIndexOrThrow(FIELD_VISIBLETYPE));
        int visibleListId = cursor.getInt(cursor
                .getColumnIndexOrThrow(FIELD_VISIBLELISTID));
...
//        JSONArray picUrlsArray = JsonUtils.str2JSONArray(pic_urls);
//        if (picUrlsArray != null && picUrlsArray.length() > 0) {
//            int length = picUrlsArray.length();
//            picUrls = new ArrayList<String>(length);
//            JSONObject tmpObject = null;
//            for (int ix = 0; ix < length; ix++) {
//                tmpObject = picUrlsArray.optJSONObject(ix);
//                if (tmpObject != null) {
//                    picUrls.add(tmpObject.optString("thumbnail_pic"));
//                }
//            }
//        }
...
//        status.setAnnotations(annotations);
    ...
    public boolean insert(StatusList statusList) {
        // 使用事务插入速度最快，使用基本sql语句次之 使用ContentValues最慢
        // long before = System.nanoTime();
        try {
            mDb.beginTransaction();
            for (Status s : statusList.statusList) {

                ContentValues values = new ContentValues();

                values.put(FIELD_USERID, s.user.id);
                values.put(FIELD_STATUSID, s.id);
                values.put(FIELD_CREATEDAT, s.created_at);
                values.put(FIELD_MID, s.mid);
                values.put(FIELD_IDSTR, s.idstr);
                values.put(FIELD_TEXT, s.text);

                values.put(FIELD_FAVORITED, s.favorited);
                values.put(FIELD_TRUNCATED, s.truncated);
                values.put(FIELD_INREPLYTOSTATUSID,
                        s.in_reply_to_status_id);
                values.put(FIELD_INREPLYTOUSERID,
                        s.in_reply_to_user_id);
                values.put(FIELD_INREPLYTOSCREENNAME,
                        s.in_reply_to_screen_name);
                values.put(FIELD_THUMBNAILPIC, s.thumbnail_pic);
                values.put(FIELD_BMIDDLEPIC, s.bmiddle_pic);
                values.put(FIELD_ORIGINALPIC, s.original_pic);

                Status s2 = s.retweeted_status;
                if (s2 != null) {
                    values.put(FIELD_RETWEETEDSTATUSID, s2.id);

                    ContentValues values2 = new ContentValues();

                    values2.put(FIELD_USERID, s2.user.id);
                    values2.put(FIELD_STATUSID, s2.id);
                    values2.put(FIELD_CREATEDAT, s2.created_at);
                    values2.put(FIELD_MID, s2.mid);
                    values2.put(FIELD_IDSTR, s2.idstr);
                    values2.put(FIELD_TEXT, s2.text);

                    values2.put(FIELD_FAVORITED, s2.favorited);
                    values2.put(FIELD_TRUNCATED, s2.truncated);
                    values2.put(FIELD_INREPLYTOSTATUSID,
                            s2.in_reply_to_status_id);
                    values2.put(FIELD_INREPLYTOUSERID,
                            s2.in_reply_to_user_id);
                    values2.put(FIELD_INREPLYTOSCREENNAME,
                            s2.in_reply_to_screen_name);
                    values2.put(FIELD_THUMBNAILPIC,
                            s2.thumbnail_pic);
                    values2.put(FIELD_BMIDDLEPIC, s2.bmiddle_pic);
                    values2.put(FIELD_ORIGINALPIC, s2.original_pic);

                    if (s2.retweeted_status != null) {
                        values2.put(FIELD_RETWEETEDSTATUSID, s2
                                .retweeted_status.id);
                    }

//                    values2.put(FIELD_GEO, s2.getGeo());
//                    values2.put(FIELD_LATITUDE, s2.getLatitude());
//                    values2.put(FIELD_LONGITUDE, s2.getLongitude());
                    values2.put(FIELD_REPOSTSCOUNT,
                            s2.reposts_count);
                    values2.put(FIELD_COMMENTSCOUNT,
                            s2.comments_count);
//                    values2.put(FIELD_ANNOTATIONS, s2.getAnnotations());
                    values2.put(FIELD_MLEVEL, s2.mlevel);
                    values2.put(FIELD_VISIBLETYPE, s2.visible
                            .type);
                    values2.put(FIELD_VISIBLELISTID, s2.visible
                            .list_id);
                    values2.put(FIELD_PIC_URLS, ListUtils.isNullOrContainEmpty(s2.pic_urls)
                            ? "" : new Gson().toJson(s2.pic_urls));
//                    values2.put(FIELD_SOURCEURL, s2.getSource()
//                            .getUrl());
//                    values2.put(FIELD_SOURCERELATIONSHIP, s2
//                            .getSource().getRelationship());
                    values2.put(FIELD_SOURCENAME, s2.source);

                    DBRetweetedStatus dbRetweetedStatus = new DBRetweetedStatus(
                            context);

                    dbRetweetedStatus.insert(values2);

                    DBUser dbUser = new DBUser(context);
                    dbUser.insertWithSQL(s2.user);
                    // mDb.insert(TABLE_NAME, null, values2);

                }
//                values.put(FIELD_GEO, s.getGeo());
//                values.put(FIELD_LATITUDE, s.getLatitude());
//                values.put(FIELD_LONGITUDE, s.getLongitude());
                values.put(FIELD_REPOSTSCOUNT, s.reposts_count);
                values.put(FIELD_COMMENTSCOUNT, s.comments_count);
//                values.put(FIELD_ANNOTATIONS, s.getAnnotations());
                values.put(FIELD_MLEVEL, s.mlevel);
                values.put(FIELD_VISIBLETYPE, s.visible.type);
                values.put(FIELD_VISIBLELISTID, s.visible
                        .list_id);

                values.put(FIELD_PIC_URLS, ListUtils.isNullOrContainEmpty(s.pic_urls)
                        ? "" : new Gson().toJson(s.pic_urls));

//                values.put(FIELD_SOURCEURL, s.getSource().getUrl());
//                values.put(FIELD_SOURCERELATIONSHIP, s.getSource()
//                        .getRelationship());
                values.put(FIELD_SOURCENAME, s.source);
    ...
    public int delete(String id) {
        ...
        return mDb.delete(TABLE_NAME, where, whereValue);
    }
    ...
    public DBStatus open() throws SQLException {
        closeConnection();
        mDbHelper = new DBStatus(context);
