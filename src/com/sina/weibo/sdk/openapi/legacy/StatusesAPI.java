...
public class StatusesAPI extends AbsOpenAPI {
    ...
    /**
     * 分享一条新微博(连续两次发布的微博不可以重复)。
     *
     * @param content  要发布的微博文本内容，内容不超过140个汉字
     * @param bitmap   用户想要分享到微博的图片，仅支持JPEG、GIF、PNG图片，上传图片大小限制为<5M。上传图片时，POST方式提交请求，需要采用multipart/form-data编码方式。
     * @param ip       非必传 开发者上报的操作用户真实IP，形如：211.156.0.1。
     * @param listener 异步请求回调接口
     */
    public void share(String content, @Nullable Bitmap bitmap, @Nullable String ip, RequestListener listener) {
        WeiboParameters params = buildUpdateParams(content, null, null);
        if (bitmap != null && bitmap.getByteCount() > 0) {
            params.put("pic", bitmap);
        }
        params.put("rip", ip);
        requestAsync(SERVER_URL_PRIX + "/share.json", params, HTTPMETHOD_POST, listener);
    }
